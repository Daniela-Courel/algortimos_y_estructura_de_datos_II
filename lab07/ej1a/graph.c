...
static unsigned int g_index(vertex_t fila, vertex_t columna, unsigned int max_vertexs) {
    return fila + columna * max_vertexs;
}

static bool invrep(graph_t g) {
    bool valid_graph = true;
    for (unsigned int i = 0; i < g->max_vertexs; i++) {
        valid_graph = g->costs[g_index(i,i,g->max_vertexs)] == 0;
    }
    
    return g!=NULL && valid_graph;
}
